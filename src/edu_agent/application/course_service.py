...
# §32：只有明确的自然语言句式才触发解析。纯课程标题（如
# "PyTorch 深度学习入门" / "Python 数据分析进阶"）不能因含"入门/进阶/学习"
# 这类词就被当作一句话解析——那会把标题错误收敛成"入门"/"进阶"。
# 仅保留强意图/时间/目标信号。
_NL_RE = re.compile(
    r"(我想|我要|我需要|我准备|希望能够|目标是|学习目标(?:是|：|:)|"
    r"每天|每日|每周|星期|周\s*(?:内|时间|学|上完|计划)|"
    r"\d+\s*(?:天|周|星期)|小时|分钟|零基础|从零|学会后|能够)"
)
