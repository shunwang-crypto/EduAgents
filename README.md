...
**课程 + 个性化学习计划 + 学习地图 + 结构化讲解 + 普通 AI 对话** —— 基于动态知识图谱与学习者模型的智能学习助手。
...
2. **学习计划（Adaptive Learning Plan & Explanation）**
   - **PlanBrief**：解释「为什么这样安排」（目标 / 关键路径 / 已确认掌握 / 建议加强 / 尚待评估 / 计划如何调整）
   - **Learning Map（学习地图）**：动态知识图谱 + 学习者状态叠加，核心导航；支持「当前学习路径 / 完整知识图」两种模式；仅一个「当前推荐」知识点；默认只聚焦当前知识点及其前后相邻节点
   - **Adaptive Rich Explanation（自适应丰富讲解）**：点击「开始讲解」进入**独立讲解页** `/courses/{courseId}/learn/{stepId}`（长文档 + 目录导航 + 自然滚动，不是卡片翻页）；按知识点复杂度动态决定篇幅与结构；支持长文、图示、图片、代码逐行讲解、表格、公式、案例、误区与应用。block/schema 只描述前端渲染方式，不限制内容长度或 section 数量
   - **职责划分**：学习地图 = 怎么学 / 现在在哪 / 为什么推荐；计划列表 = 顺序 / 时间 / 进度；独立讲解页 = 真正学知识内容。讲解页底部 `完成本节讲解` 只更新进度，**不修改 mastery**
   - **Practice Handoff**：仅提供「进入相关实践」入口（接口契约，不实现练习）
3. **普通 AI 对话** — 无课程普通对话 / 带课程上下文 / 带计划步骤上下文（PlanStepContext）

后台保留轻量 **Dynamic Learner Model**（SQLite 唯一画像真值），让计划更贴合背景、对话更个性化。
**不做**：Quiz / Practice / Grading / Assessment / Mistake、排行榜、社交功能、新 Agent 等（练习 / 评测属于外部 Practice 模块职责）。
