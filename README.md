...
2. **学习计划** — 每门课程一个个性化计划（固定三阶段：基础准备 / 核心学习 / 综合应用）；每步可「开始学习 / 继续学习」展开**懒生成讲解**（首次进入时按需调用 LLM 生成，缓存复用，不污染计划生成），可「标记完成」；计划可设置**学习周期 / 每日时长**（生成与重新生成沿用为默认值）；每步可「就此提问」进入知识点上下文对话；GPT 风格 Rich Markdown（表格 / 代码 / LaTeX 数学）
