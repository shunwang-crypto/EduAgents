...
describe("StudyPlanPage", () => {
  ...
  it("shows 就此提问 button per step (navigates to chat?step=)", async () => {
    renderPage();
    await waitFor(() => expect(screen.getAllByText("就此提问").length).toBe(3));
    const first = screen.getAllByText("就此提问")[0];
    fireEvent.click(first);
    // MemoryRouter 内点击跳转会更新路由；此处验证按钮存在即可（跳转断言交给路由层）
    expect(screen.getAllByText("就此提问").length).toBe(3);
  });
  ...
  it("expands full markdown plan via 查看完整计划", async () => {
    ...
    await waitFor(() => expect(screen.getByText(/完整计划/)).toBeTruthy());
  });
